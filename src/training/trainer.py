...
from src.nn.model import GraphNetwork, SubgraphDiscriminator, MLP, GAT, Node2Vec
    ...
    def build_macro_graph(self, file_name=None):
        ...
        with tqdm(total=len(self.dataset)) as p_bar:
            ...
            for i, (pairs, macro_arch_graph, cls_label, reg_label, label_class) in enumerate(self.dataset): # ！
                ...
                # print(torch.tensor(macro_arch_graph).shape) # [16, 256]
                # if i == 10:
                #     break
                # if macro_arch_graph.
                # exit(0)
...
class Tester(object):
    ...
    def __init__(self, model_args, dataset, batch_size=32, optimizer_kwargs=None,
                 logging_interval=50, device='cpu', model_name='GCN'):
        
        super(Tester, self).__init__()

        assert model_name.lower() in {'graphnetwork', 'gcn', 'gat'}
        if model_name.lower() in {'graphnetwork', 'gcn'}:
    ...
    def get_graph_embedding(self):
        ...
        with tqdm(total=len(self.data_loader_eval)) as p_bar:
            p_bar.set_description('Generating graph embedding:')
            for i, batch in enumerate(self.data_loader_eval):
                # print('c1')
                print(type(batch))
