...
from torch_geometric.nn import GCNConv, global_mean_pool, Node2Vec, GATConv
...
class AutoEncoder(nn.Module):
    def __init__(self, input_dim, emb_dim, hidden_dim_array = [256, 128], non_linear_function_array=[nn.ReLU, nn.ReLU], drop_rate=0., **kwargs):
        super().__init__()
        self.encode_nodes = GraphNetwork(in_channelshidden_channels=hidden_dim_array[0], hidden_channels=hidden_dim_array[0], num_layers=1)
    ...
    def forward(self, x, edges): # graph -> [N, K]
        x = self.encode_nodes(x, edges)
        emb = self.encoder(x)
        if self._return_embedding:
            return emb
        x = self.decoder(emb)
...
class GAT(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, num_layers, num_regs=None, num_classes=None,
                 gnn_operator=GATConv, drop_rate=0., **kwargs):
